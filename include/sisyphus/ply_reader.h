//
// Created by kganguly on 5/1/17.
//

#ifndef PLY_READER_CUSTOM_PLY_READER_H
#define PLY_READER_CUSTOM_PLY_READER_H
#ifndef tinyply_h
#define tinyply_h
...
namespace ply_reader {
    ...
    template<typename T>
    T endian_swap(const T &v) { return v; }

    template<>
    inline uint16_t endian_swap(const uint16_t &v) { return (v << 8) | (v >> 8); }
    ...
    void ply_cast(void *dest, const char *src, bool be) {
        *(static_cast<T *>(dest)) = (be) ? endian_swap(*(reinterpret_cast<const T *>(src)))
                                         : *(reinterpret_cast<const T *>(src));
    }

    template<typename T>
    void ply_cast_float(void *dest, const char *src, bool be) {
        *(static_cast<T *>(dest)) = (be) ? endian_swap_float(*(reinterpret_cast<const uint32_t *>(src)))
                                         : *(reinterpret_cast<const T *>(src));
    }

    template<typename T>
    void ply_cast_double(void *dest, const char *src, bool be) {
        *(static_cast<T *>(dest)) = (be) ? endian_swap_double(*(reinterpret_cast<const uint64_t *>(src)))
                                         : *(reinterpret_cast<const T *>(src));
    }
    ...
    public:
        ...
        size_t request_properties_from_element(const std::string &elementKey, std::vector<std::string> propertyKeys,
            ...
            if (find_element(elementKey, get_elements()) >= 0) {
                if (std::find(requestedElements.begin(), requestedElements.end(), elementKey) ==
            ...
            for (auto key : propertyKeys) {
                if (int instanceCount = instance_counter(elementKey, key)) {
                    ...
                    auto result = userDataTable.insert(
                            std::pair<std::string, std::shared_ptr<DataCursor>>(make_key(elementKey, key), cursor));
            ...
            size_t totalInstanceSize = [&]() {
                size_t t = 0;
                for (auto c : instanceCounts) { t += c; }
                return t;
            }() * listCount;
            source.resize(
                    totalInstanceSize); // this satisfies regular properties; `cursor->realloc` is for list types since tinyply uses single-pass parsing
            cursor->offset = 0;
        ...
        void add_properties_to_element(const std::string &elementKey, const std::vector<std::string> &propertyKeys,
            ...
            auto create_property_on_element = [&](PlyElement &e) {
                for (auto key : propertyKeys) {
                    ...
                    PlyProperty newProp = (listType == PlyProperty::Type::INVALID) ? PlyProperty(t, key) : PlyProperty(
                            listType, t, key, listCount);
            ...
            if (idx >= 0) {
                ...
                PlyElement newElement = (listCount == 1) ? PlyElement(elementKey, source.size() / propertyKeys.size())
...
#endif // tinyply_h
#endif //PLY_READER_CUSTOM_PLY_READER_H
