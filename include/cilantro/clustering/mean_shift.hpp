...
public:
  ...
  MeanShift& cluster(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& seeds, ScalarT kernel_radius,
                     size_t max_iter, ScalarT cluster_tol,
...
#pragma omp parallel for shared(has_converged, all_converged) private(nn, point_tmp)
      for (size_t i = 0; i < shifted_seeds_.cols(); i++) {
        ...
        for (size_t j = 0; j < nn.size(); j++) {
          const ScalarT weight =
              evaluator.template operator()<Eigen::Ref<const Vector<ScalarT, EigenDim>>>(
                  shifted_seeds_.col(i), data_map_.col(nn[j].index), nn[j].value);
    ...
    for (size_t i = 0; i < shifted_seeds_.cols(); i++) {
      ...
      for (c = 0; c < this->cluster_to_point_indices_map_.size(); c++) {
        if ((shifted_seeds_.col(i) - shifted_seeds_.col(this->cluster_to_point_indices_map_[c][0]))
                .squaredNorm() < cluster_tol_sq)
          break;
      }
    ...
    for (size_t i = 0; i < this->cluster_to_point_indices_map_.size(); i++) {
      ...
      for (size_t j = 0; j < this->cluster_to_point_indices_map_[i].size(); j++) {
        ...
        this->point_to_cluster_index_map_[this->cluster_to_point_indices_map_[i][j]] =
            static_cast<ClusterIndexT>(i);
      }
  ...
  template <class KernelEvaluatorT = UnityWeightEvaluator<ScalarT, ScalarT>>
  inline MeanShift& cluster(ScalarT kernel_radius, size_t max_iter, ScalarT cluster_tol,
                            ScalarT convergence_tol = std::numeric_limits<ScalarT>::epsilon(),
                            const KernelEvaluatorT& evaluator = KernelEvaluatorT()) {
    return cluster<KernelEvaluatorT>(data_map_, kernel_radius, max_iter, cluster_tol,
                                     convergence_tol, evaluator);
  }
