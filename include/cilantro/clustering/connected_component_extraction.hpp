...
template <typename ScalarT, typename IndexT,
          class PointSimilarityEvaluator = AlwaysTrueEvaluator<ScalarT>>
void extractConnectedComponents(
    const NeighborhoodSet<ScalarT, IndexT>& neighbors, const std::vector<IndexT>& seeds_ind,
    std::vector<std::vector<IndexT>>& segment_to_point_map,
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
...
#pragma omp parallel for shared(seeds_ind, current_label, seed_active, \
  ...
  for (size_t i = 0; i < segment_to_point_map_tmp.size(); i++) {
    if (segment_to_point_map_tmp[i].size() >= min_segment_size &&
        segment_to_point_map_tmp[i].size() <= max_segment_size) {
      segment_to_point_map.emplace_back(std::move(segment_to_point_map_tmp[i]));
    }
  }

  std::sort(segment_to_point_map.begin(), segment_to_point_map.end(),
            SizeGreaterComparator<std::vector<IndexT>>());
}

// Given neighbors and seeds
template <typename ScalarT, typename IndexT,
          class PointSimilarityEvaluator = AlwaysTrueEvaluator<ScalarT>>
inline std::vector<std::vector<IndexT>> extractConnectedComponents(
    const NeighborhoodSet<ScalarT, IndexT>& neighbors, const std::vector<IndexT>& seeds_ind,
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
  std::vector<std::vector<IndexT>> segment_to_point_map;
  extractConnectedComponents<ScalarT, IndexT, PointSimilarityEvaluator>(
      neighbors, seeds_ind, segment_to_point_map, evaluator, min_segment_size, max_segment_size);
...
template <typename ScalarT, typename IndexT,
          class PointSimilarityEvaluator = AlwaysTrueEvaluator<ScalarT>>
void extractConnectedComponents(
    const NeighborhoodSet<ScalarT, IndexT>& neighbors,
    std::vector<std::vector<IndexT>>& segment_to_point_map,
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
...
template <typename ScalarT, typename IndexT,
          class PointSimilarityEvaluator = AlwaysTrueEvaluator<ScalarT>>
std::vector<std::vector<IndexT>> extractConnectedComponents(
    const NeighborhoodSet<ScalarT, IndexT>& neighbors,
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
  std::vector<IndexT> seeds_ind(neighbors.size());
...
template <typename ScalarT, ptrdiff_t EigenDim, template <class> class DistAdaptor, typename IndexT,
          class NeighborhoodSpecT, class PointSimilarityEvaluator = AlwaysTrueEvaluator<ScalarT>>
void extractConnectedComponents(
    const KDTree<ScalarT, EigenDim, DistAdaptor, IndexT>& tree, const NeighborhoodSpecT& nh,
    const std::vector<IndexT>& seeds_ind, std::vector<std::vector<IndexT>>& segment_to_point_map,
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
  constexpr size_t unassigned = std::numeric_limits<size_t>::max();
...
inline std::vector<std::vector<IndexT>> extractConnectedComponents(
    ...
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
  std::vector<std::vector<IndexT>> segment_to_point_map;
  extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, IndexT, NeighborhoodSpecT,
                             PointSimilarityEvaluator>(
      tree, nh, seeds_ind, segment_to_point_map, evaluator, min_segment_size, max_segment_size);
  return segment_to_point_map;
}
...
template <typename ScalarT, ptrdiff_t EigenDim, template <class> class DistAdaptor, typename IndexT,
          class NeighborhoodSpecT, class PointSimilarityEvaluator = AlwaysTrueEvaluator<ScalarT>>
inline void extractConnectedComponents(
    const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points, const NeighborhoodSpecT& nh,
    const std::vector<IndexT>& seeds_ind, std::vector<std::vector<IndexT>>& segment_to_point_map,
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
  extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, IndexT, NeighborhoodSpecT,
                             PointSimilarityEvaluator>(
      KDTree<ScalarT, EigenDim, DistAdaptor, IndexT>(points), nh, seeds_ind, segment_to_point_map,
      evaluator, min_segment_size, max_segment_size);
}
...
inline std::vector<std::vector<IndexT>> extractConnectedComponents(
    ...
    const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
    size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
  return extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, IndexT, NeighborhoodSpecT,
                                    PointSimilarityEvaluator>(
      KDTree<ScalarT, EigenDim, DistAdaptor, IndexT>(points), nh, seeds_ind, evaluator,
...
inline void extractConnectedComponents(
  ...
  extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, IndexT, NeighborhoodSpecT,
                             PointSimilarityEvaluator>(
      KDTree<ScalarT, EigenDim, DistAdaptor, IndexT>(points), nh, segment_to_point_map, evaluator,
      min_segment_size, max_segment_size);
}
...
inline std::vector<std::vector<IndexT>> extractConnectedComponents(
  ...
  return extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, IndexT, NeighborhoodSpecT,
                                    PointSimilarityEvaluator>(
      KDTree<ScalarT, EigenDim, DistAdaptor, IndexT>(points), nh, evaluator, min_segment_size,
      max_segment_size);
}

template <typename ScalarT, ptrdiff_t EigenDim,
          template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
...
public:
  ...
  ConnectedComponentExtraction(const SearchTree& kd_tree)
      : points_(kd_tree.getPointsMatrixMap()), kd_tree_ptr_(&kd_tree), kd_tree_owned_(false) {}
  ...
  inline ConnectedComponentExtraction& segment(
      const NeighborhoodSpecT& nh, const std::vector<PointIndexT>& seeds_ind,
      const PointSimilarityEvaluator& evaluator = PointSimilarityEvaluator(),
      size_t min_segment_size = 1, size_t max_segment_size = std::numeric_limits<size_t>::max()) {
    extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, PointIndexT, NeighborhoodSpecT,
                               PointSimilarityEvaluator>(
        *kd_tree_ptr_, nh, seeds_ind, this->cluster_to_point_indices_map_, evaluator,
        min_segment_size, max_segment_size);
    this->point_to_cluster_index_map_ =
        cilantro::getPointToClusterIndexMap<ClusterIndexT, PointIndexT>(
            this->cluster_to_point_indices_map_, points_.cols());
    return *this;
  }
  ...
  inline ConnectedComponentExtraction& segment(
    ...
    extractConnectedComponents<ScalarT, EigenDim, DistAdaptor, PointIndexT, NeighborhoodSpecT,
                               PointSimilarityEvaluator>(
        *kd_tree_ptr_, nh, this->cluster_to_point_indices_map_, evaluator, min_segment_size,
        max_segment_size);
    this->point_to_cluster_index_map_ =
        cilantro::getPointToClusterIndexMap<ClusterIndexT, PointIndexT>(
            this->cluster_to_point_indices_map_, points_.cols());
...
template <template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
using ConnectedComponentExtraction2f =
    ConnectedComponentExtraction<float, 2, DistAdaptor, PointIndexT, ClusterIndexT>;

template <template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
using ConnectedComponentExtraction2d =
    ConnectedComponentExtraction<double, 2, DistAdaptor, PointIndexT, ClusterIndexT>;

template <template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
using ConnectedComponentExtraction3f =
    ConnectedComponentExtraction<float, 3, DistAdaptor, PointIndexT, ClusterIndexT>;

template <template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
using ConnectedComponentExtraction3d =
    ConnectedComponentExtraction<double, 3, DistAdaptor, PointIndexT, ClusterIndexT>;

template <template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
using ConnectedComponentExtractionXf =
    ConnectedComponentExtraction<float, Eigen::Dynamic, DistAdaptor, PointIndexT, ClusterIndexT>;

template <template <class> class DistAdaptor = KDTreeDistanceAdaptors::L2,
          typename PointIndexT = size_t, typename ClusterIndexT = size_t>
using ConnectedComponentExtractionXd =
    ConnectedComponentExtraction<double, Eigen::Dynamic, DistAdaptor, PointIndexT, ClusterIndexT>;
