...
public:
  ...
  KMeans(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& data)
      : data_map_(data), iteration_count_(0) {}

  ~KMeans() {}

  KMeans& cluster(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& centroids,
                  size_t max_iter = 100, ScalarT tol = std::numeric_limits<ScalarT>::epsilon(),
  ...
  KMeans& cluster(size_t num_clusters, size_t max_iter = 100,
                  ScalarT tol = std::numeric_limits<ScalarT>::epsilon(), bool use_kd_tree = false) {
    cluster_centroids_.resize(
        data_map_.rows(),
        std::max(size_t(1), std::min(num_clusters, static_cast<size_t>(data_map_.cols()))));
  ...
  inline const VectorSet<ScalarT, EigenDim>& getClusterCentroids() const {
...
#pragma omp parallel for shared(assignments_unchanged) private(min_dist, min_dist_ind, dist)
        for (size_t i = 0; i < num_points; i++) {
          ...
          for (size_t j = 0; j < num_clusters; j++) {
            if constexpr (std::is_same<DistAdaptor<DataAdaptor>,
                                       KDTreeDistanceAdaptors::L2<DataAdaptor>>::value ||
                          std::is_same<DistAdaptor<DataAdaptor>,
                                       KDTreeDistanceAdaptors::L2Simple<DataAdaptor>>::value) {
      ...
      for (size_t i = 0; i < num_clusters; i++) {
        ...
        Vector<ScalarT, EigenDim> old_centroid(cluster_centroids_.col(max_ind) *
                                               (ScalarT(1.0) / point_count[max_ind]));
...
#pragma omp critical
      ...
      if (tol > ScalarT(0.0) &&
          (cluster_centroids_ - centroids_old).colwise().squaredNorm().maxCoeff() < tol_sq)
        break;
    }

    this->cluster_to_point_indices_map_ =
        cilantro::getClusterToPointIndicesMap<PointIndexT, ClusterIndexT>(
            this->point_to_cluster_index_map_, num_clusters);
  }
};
