...
public:
  SpectraDiagonalInverseBop(const Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic>& D)
      : dim_(D.rows()), diag_vals_(D.diagonal()), diag_vals_inv_(diag_vals_.cwiseInverse()) {}
...
size_t estimateNumberOfClustersEigengap(const VectorT& eigenvalues, size_t max_num_clusters) {
...
void computeLaplacianSpectralEmbedding(
    const Eigen::Ref<const Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic>>& affinities,
  ...
  const size_t num_eigenvalues =
      (estimate_num_clusters) ? std::min(max_num_clusters + 1, (size_t)(affinities.rows() - 1))
                              : max_num_clusters;
  ...
  case GraphLaplacianType::UNNORMALIZED: {
    Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic> L =
        affinities.colwise().sum().asDiagonal();
    L -= affinities;

    Spectra::DenseSymMatProd<ScalarT> op(L);
    Spectra::SymEigsSolver<Spectra::DenseSymMatProd<ScalarT>> eig(
        op, num_eigenvalues, std::min(2 * num_eigenvalues, (size_t)affinities.rows()));
    eig.init();
    do {
      n_conv = eig.compute(Spectra::SortRule::SmallestMagn, max_iter, conv_tol,
  ...
  case GraphLaplacianType::NORMALIZED_SYMMETRIC: {
    Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic> Dtm12 =
        affinities.colwise().sum().array().rsqrt().matrix().asDiagonal();
    Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic> L =
        Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic>::Identity(affinities.rows(),
                                                                         affinities.cols()) -
        Dtm12 * affinities * Dtm12;
  ...
  case GraphLaplacianType::NORMALIZED_RANDOM_WALK: {
    Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic> D =
        affinities.colwise().sum().asDiagonal();
    Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic> L = D - affinities;

    Spectra::DenseSymMatProd<ScalarT> op(L);
    internal::SpectraDiagonalInverseBop<ScalarT> Bop(D);
    Spectra::SymGEigsSolver<Spectra::DenseSymMatProd<ScalarT>,
                            internal::SpectraDiagonalInverseBop<ScalarT>,
                            Spectra::GEigsMode::RegularInverse>
        eig(op, Bop, num_eigenvalues, std::min(2 * num_eigenvalues, (size_t)affinities.rows()));
...
void computeLaplacianSpectralEmbedding(const Eigen::SparseMatrix<ScalarT>& affinities,
                                       size_t max_num_clusters, bool estimate_num_clusters,
  ...
  case GraphLaplacianType::UNNORMALIZED: {
    ...
    Spectra::SymEigsSolver<Spectra::SparseSymMatProd<ScalarT>> eig(
        op, num_eigenvalues, std::min(2 * num_eigenvalues, (size_t)affinities.rows()));
    eig.init();
    do {
      n_conv = eig.compute(Spectra::SortRule::SmallestMagn, max_iter, conv_tol,
  ...
  case GraphLaplacianType::NORMALIZED_RANDOM_WALK: {
    ...
    Spectra::SparseSymMatProd<ScalarT> op(L);
    internal::SpectraDiagonalInverseBop<ScalarT> Bop(D);
    Spectra::SymGEigsSolver<Spectra::SparseSymMatProd<ScalarT>,
                            internal::SpectraDiagonalInverseBop<ScalarT>,
                            Spectra::GEigsMode::RegularInverse>
        eig(op, Bop, num_eigenvalues, std::min(2 * num_eigenvalues, (size_t)affinities.rows()));
...
// If positive, EigenDim is the embedding dimension (and also the number of clusters).
// Set to Eigen::Dynamic for runtime setting.
template <typename ScalarT, ptrdiff_t EigenDim = Eigen::Dynamic, typename PointIndexT = size_t,
          typename ClusterIndexT = size_t>
class SpectralClustering
    : public SpectralEmbeddingBase<SpectralClustering<ScalarT, EigenDim>, ScalarT, EigenDim>,
      public KMeans<ScalarT, EigenDim, KDTreeDistanceAdaptors::L2, PointIndexT, ClusterIndexT> {
  typedef SpectralEmbeddingBase<SpectralClustering<ScalarT, EigenDim>, ScalarT, EigenDim>
      EmbeddingBase;
...
public:
  ...
  SpectralClustering(
      ...
      : EmbeddingBase(),
        Clusterer((computeLaplacianSpectralEmbedding<ScalarT, EigenDim>(
                       affinities, EigenDim, false, laplacian_type, this->embedded_points_,
                       this->computed_eigenvalues_),
                   this->embedded_points_)) {
    this->cluster(this->embedded_points_.rows(), kmeans_max_iter, kmeans_conv_tol,
                  kmeans_use_kd_tree);
  }

  // Dense input
  // Number of clusters (embedding dimension) set at runtime (EigenDim == Eigen::Dynamic)
  // If estimate_num_clusters == true, chooses number of clusters in [1, max_num_clusters] based on
  // eigenvalue distribution. Otherwise, returns max_num_clusters clusters.
  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim == Eigen::Dynamic>::type>
  SpectralClustering(
      const Eigen::Ref<const Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic>>& affinities,
      size_t max_num_clusters, bool estimate_num_clusters = false,
      const GraphLaplacianType& laplacian_type = GraphLaplacianType::NORMALIZED_RANDOM_WALK,
      size_t kmeans_max_iter = 100,
      ScalarT kmeans_conv_tol = std::numeric_limits<ScalarT>::epsilon(),
      bool kmeans_use_kd_tree = false)
      : EmbeddingBase(),
        Clusterer(((max_num_clusters > 0 && max_num_clusters < affinities.rows())
                       ? computeLaplacianSpectralEmbedding<ScalarT, EigenDim>(
                             affinities, max_num_clusters, estimate_num_clusters, laplacian_type,
                             this->embedded_points_, this->computed_eigenvalues_)
                       : computeLaplacianSpectralEmbedding<ScalarT, EigenDim>(
                             affinities, 2, false, laplacian_type, this->embedded_points_,
                             this->computed_eigenvalues_),
                   this->embedded_points_)) {
    this->cluster(this->embedded_points_.rows(), kmeans_max_iter, kmeans_conv_tol,
                  kmeans_use_kd_tree);
  ...
  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim != Eigen::Dynamic>::type>
  SpectralClustering(
      const Eigen::SparseMatrix<ScalarT>& affinities,
      const GraphLaplacianType& laplacian_type = GraphLaplacianType::NORMALIZED_RANDOM_WALK,
      size_t kmeans_max_iter = 100,
      ScalarT kmeans_conv_tol = std::numeric_limits<ScalarT>::epsilon(),
      bool kmeans_use_kd_tree = false)
  ...
  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim == Eigen::Dynamic>::type>
  SpectralClustering(
      const Eigen::SparseMatrix<ScalarT>& affinities, size_t max_num_clusters,
