...
namespace cilantro {
    ...
    public:
        ...
        void nearestNeighborSearch(const Eigen::Ref<const Eigen::Matrix<ScalarT,EigenDim,1>> &query_pt, size_t &neighbor, ScalarT &distance) const {
            kd_tree_.knnSearch(query_pt.data(), 1, &neighbor, &distance);
        }

        void kNNSearch(const Eigen::Ref<const Eigen::Matrix<ScalarT,EigenDim,1>> &query_pt, size_t k, std::vector<size_t> &neighbors, std::vector<ScalarT> &distances) const {
            neighbors.resize(k);
            distances.resize(k);
        ...
        void kNNInRadiusSearch(const Eigen::Ref<const Eigen::Matrix<ScalarT,EigenDim,1>> &query_pt, size_t k, ScalarT radius, std::vector<size_t> &neighbors, std::vector<ScalarT> &distances) const {
...
//            }
        ...
        inline void search(const Eigen::Ref<const Eigen::Matrix<ScalarT,EigenDim,1>> &query_pt, std::vector<size_t> &neighbors, std::vector<ScalarT> &distances, const Neighborhood &nh) const {
            switch (nh.type) {
