...
public:
  ...
  FlatConvexHull3(const ConstVectorSetMatrixMap<ScalarT, 3>& points, bool compute_topology = false,
      ...
      : PrincipalComponentAnalysis<ScalarT, 3>(points),
        ConvexHull<ScalarT, 2, IndexT>(this->template project<2>(points), compute_topology,
  ...
  FlatConvexHull3& transform(const Eigen::Ref<const Eigen::Matrix<ScalarT, 3, 3>>& rotation,
    ...
    proj_mat.topRightCorner(2, 1).noalias() =
        -this->eigenvectors_.leftCols(2).transpose() * this->mean_;
  ...
  inline FlatConvexHull3& transform(
      const Eigen::Ref<const Eigen::Matrix<ScalarT, 4, 4>>& rigid_transform) {
