...
public:
  ...
  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim == Eigen::Dynamic>::type>
  ConvexPolytope(size_t dim = 2)
      : dim_(dim), is_empty_(true), is_bounded_(true), area_(0.0), volume_(0.0) {
  ...
  ConvexPolytope(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points,
                 bool compute_topology = false, bool simplicial_facets = false,
  ...
  ConvexPolytope(const ConstHomogeneousVectorSetMatrixMap<ScalarT, EigenDim>& halfspaces,
                 bool compute_topology = false, bool simplicial_facets = false,
                 double merge_tol = 0.0, double dist_tol = std::numeric_limits<ScalarT>::epsilon())
      : dim_(EigenDim) {
    init_halfspaces_(halfspaces, compute_topology, simplicial_facets, merge_tol, dist_tol);
  }

  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim == Eigen::Dynamic>::type>
  ConvexPolytope(const ConstDataMatrixMap<ScalarT, EigenDim>& input_data, size_t dim,
                 bool compute_topology = false, bool simplicial_facets = false,
                 double merge_tol = 0.0, double dist_tol = std::numeric_limits<ScalarT>::epsilon())
      : dim_(dim) {
  ...
  ConvexPolytope intersectionWith(const ConvexPolytope& poly, bool compute_topology = false,
    ...
    if constexpr (EigenDim != Eigen::Dynamic) {
      HomogeneousVectorSet<ScalarT, EigenDim> hs_intersection(
          EigenDim + 1, halfspaces_.cols() + poly.halfspaces_.cols());
      hs_intersection.leftCols(halfspaces_.cols()) = halfspaces_;
      hs_intersection.rightCols(poly.halfspaces_.cols()) = poly.halfspaces_;
      return ConvexPolytope(hs_intersection, compute_topology, simplicial_facets, merge_tol,
                            dist_tol);
    } else {
      HomogeneousVectorSet<ScalarT, EigenDim> hs_intersection(
          dim_ + 1, halfspaces_.cols() + poly.halfspaces_.cols());
      hs_intersection.leftCols(halfspaces_.cols()) = halfspaces_;
      hs_intersection.rightCols(poly.halfspaces_.cols()) = poly.halfspaces_;
      return ConvexPolytope(hs_intersection, dim_, compute_topology, simplicial_facets, merge_tol,
                            dist_tol);
    }
  }
  ...
  inline const HomogeneousVectorSet<ScalarT, EigenDim>& getFacetHyperplanes() const {
    return halfspaces_;
  }

  inline const Vector<ScalarT, EigenDim>& getInteriorPoint() const { return interior_point_; }

  inline bool containsPoint(const Eigen::Ref<const Vector<ScalarT, EigenDim>>& point,
  ...
  inline Eigen::Matrix<ScalarT, Eigen::Dynamic, Eigen::Dynamic> getPointSignedDistancesFromFacets(
      const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points) const {
    return (halfspaces_.topRows(dim_).transpose() * points).colwise() +
           halfspaces_.row(dim_).transpose();
  }
  ...
  inline const std::vector<std::vector<IndexT>>& getVertexNeighborFacets() const {
    return vertex_neighbor_faces_;
  }

  inline const std::vector<std::vector<IndexT>>& getFacetNeighborFacets() const {
    return face_neighbor_faces_;
  }

  inline const std::vector<IndexT>& getVertexPointIndices() const { return vertex_point_indices_; }

  ConvexPolytope& transform(
    ...
    typename std::conditional<
        EigenDim == Eigen::Dynamic, Eigen::Matrix<ScalarT, EigenDim, EigenDim>,
        Eigen::Matrix<ScalarT, EigenDim + 1, EigenDim + 1>>::type hs_tform(dim_ + 1, dim_ + 1);
  ...
  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim != Eigen::Dynamic>::type>
  inline ConvexPolytope& transform(
      const Eigen::Ref<const Eigen::Matrix<ScalarT, EigenDim + 1, EigenDim + 1>>& tform) {
    return transform(tform.template topLeftCorner<EigenDim, EigenDim>(),
                     tform.template topRightCorner<EigenDim, 1>());
  }

  template <ptrdiff_t Dim = EigenDim, class = typename std::enable_if<Dim == Eigen::Dynamic>::type>
  inline ConvexPolytope& transform(
      const Eigen::Ref<const Eigen::Matrix<ScalarT, EigenDim, EigenDim>>& tform) {
...
protected:
  ...
  inline void init_points_(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points,
                           bool compute_topology, bool simplicial_facets, double merge_tol) {
    is_empty_ = (compute_topology)
                    ? !convexHullFromPoints<ScalarT, EigenDim, IndexT>(
                          points, vertices_, halfspaces_, faces_, vertex_neighbor_faces_,
                          face_neighbor_faces_, vertex_point_indices_, area_, volume_,
                          simplicial_facets, merge_tol)
                    : !halfspaceIntersectionFromVertices<ScalarT, EigenDim>(
                          points, vertices_, halfspaces_, area_, volume_, true, merge_tol);
    is_bounded_ = true;
    if (is_empty_) {
  ...
  inline void init_halfspaces_(
      const ConstHomogeneousVectorSetMatrixMap<ScalarT, EigenDim>& halfspaces,
      bool compute_topology, bool simplicial_facets, double merge_tol, double dist_tol) {
    is_empty_ = !evaluateHalfspaceIntersection<ScalarT, EigenDim>(
        halfspaces, halfspaces_, vertices_, interior_point_, is_bounded_, dist_tol, merge_tol);
    if (is_empty_) {
      area_ = 0.0;
      volume_ = 0.0;
    } else if (is_bounded_) {
      if (compute_topology) {
        is_empty_ = !convexHullFromPoints<ScalarT, EigenDim, IndexT>(
            vertices_, vertices_, halfspaces_, faces_, vertex_neighbor_faces_, face_neighbor_faces_,
            vertex_point_indices_, area_, volume_, simplicial_facets, merge_tol);
        if (is_empty_) {
          interior_point_.setConstant(dim_, 1, std::numeric_limits<ScalarT>::quiet_NaN());
        } else {
          interior_point_ = vertices_.rowwise().mean();
