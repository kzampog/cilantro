...
namespace cilantro {
    ...
    public:
        ...
        VectorSet<ScalarT,EigenDim> estimateNormalsKNN(size_t num_neighbors) const {
            size_t dim = points_.rows();
            size_t num_points = points_.cols();

            VectorSet<ScalarT,EigenDim> normals(dim, num_points);
            Vector<ScalarT,EigenDim> nan(Vector<ScalarT,EigenDim>::Constant(dim, 1, std::numeric_limits<ScalarT>::quiet_NaN()));
            if (num_points < dim) {
...
#pragma omp parallel for shared (normals) private (neighbors, distances)
        ...
        VectorSet<ScalarT,EigenDim> estimateNormalsRadius(ScalarT radius) const {
            size_t dim = points_.rows();
            size_t num_points = points_.cols();
...
#pragma omp parallel for shared (normals) private (neighbors, distances)
        ...
        VectorSet<ScalarT,EigenDim> estimateNormalsKNNInRadius(size_t k, ScalarT radius) const {
            size_t dim = points_.rows();
            size_t num_points = points_.cols();
            ScalarT radius_sq = radius*radius;
...
#pragma omp parallel for shared (normals) private (neighbors, distances)
        ...
        VectorSet<ScalarT,EigenDim> estimateNormals(const typename KDTree<ScalarT,EigenDim,KDTreeDistanceAdaptors::L2>::Neighborhood &nh) const {
            switch (nh.type) {
