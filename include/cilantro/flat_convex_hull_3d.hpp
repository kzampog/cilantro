...
namespace cilantro {
    ...
    class FlatConvexHull3D : public PrincipalComponentAnalysis<ScalarT,3>, public ConvexHull<ScalarT,2> {
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW

        FlatConvexHull3D(const ConstVectorSetMatrixMap<ScalarT,3> &points, bool compute_topology = false, bool simplicial_facets = false, double merge_tol = 0.0)
        ...
        FlatConvexHull3D& transform(const Eigen::Ref<const Eigen::Matrix<ScalarT,3,3>> &rotation, const Eigen::Ref<const Eigen::Matrix<ScalarT,3,1>> &translation) {
            ...
            ConvexHull2f::transform(proj_mat*tform*rec_mat);

            return *this;
        }

        inline FlatConvexHull3D& transform(const Eigen::Ref<const Eigen::Matrix<ScalarT,4,4>> &rigid_transform) {
