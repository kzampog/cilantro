...
template <class TransformT, class CorrespondenceSearchEngineT,
          ...
          class PointToPlaneCorrWeightEvaluatorT =
              UnityWeightEvaluator<typename TransformT::Scalar, typename TransformT::Scalar>>
class CombinedMetricSingleTransformICP
...
public:
  ...
  CombinedMetricSingleTransformICP(
      ...
      CorrespondenceSearchEngineT& corr_engine, PointToPointCorrWeightEvaluatorT& point_corr_eval,
      ...
      : Base(corr_engine),
        ...
        optimization_convergence_tol_((typename TransformT::Scalar)1e-5),
        ...
        dst_mean_((dst_points_.cols() == 0)
                      ? Vector<typename TransformT::Scalar, TransformT::Dim>::Zero()
                      : Vector<typename TransformT::Scalar, TransformT::Dim>(
                            dst_points_.rowwise().mean())),
        src_mean_((src_points_.cols() == 0)
                      ? Vector<typename TransformT::Scalar, TransformT::Dim>::Zero()
                      : Vector<typename TransformT::Scalar, TransformT::Dim>(
                            src_points_.rowwise().mean())),
  ...
  inline CombinedMetricSingleTransformICP& setPointToPointMetricWeight(
  ...
  inline CombinedMetricSingleTransformICP& setPointToPlaneMetricWeight(
      typename TransformT::Scalar weight) {
    point_to_plane_weight_ = weight;
    return *this;
  }

  inline size_t getMaxNumberOfOptimizationStepIterations() const {
    return max_optimization_iterations_;
  }

  inline CombinedMetricSingleTransformICP& setMaxNumberOfOptimizationStepIterations(
      size_t max_iter) {
    max_optimization_iterations_ = max_iter;
    return *this;
  }

  inline typename TransformT::Scalar getOptimizationStepConvergenceTolerance() const {
    return optimization_convergence_tol_;
  }

  inline CombinedMetricSingleTransformICP& setOptimizationStepConvergenceTolerance(
...
private:
  ...
  void updateEstimate() {
    ...
    if constexpr (int(Base::Transform::Mode) == int(Eigen::Isometry) &&
                  (Base::Transform::Dim == 3 || Base::Transform::Dim == 2)) {
      if (has_source_normals_) {
        transformNormals(this->transform_, src_normals_, src_normals_trans_);
        estimateTransformSymmetricMetric(
            dst_points_, dst_normals_, src_points_trans_, src_normals_trans_,
            corr_getter_proxy.getPointToPointCorrespondences(), point_to_point_weight_,
            corr_getter_proxy.getPointToPlaneCorrespondences(), point_to_plane_weight_, tform_iter,
            max_optimization_iterations_, optimization_convergence_tol_, point_corr_eval_,
            plane_corr_eval_, dst_mean_, this->transform_ * src_mean_);
      } else {
...
#pragma omp parallel for shared(res) private(nn, src_p_trans)
    for (size_t i = 0; i < src_points_.cols(); i++) {
      ...
      typename TransformT::Scalar point_to_plane_dist =
          normal.dot(dst_points_.col(nn.index) - src_p_trans);
...
using CombinedMetricRigidTransformICP2f =
    CombinedMetricSingleTransformICP<RigidTransform<float, 2>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
using CombinedMetricRigidTransformICP2d =
    CombinedMetricSingleTransformICP<RigidTransform<double, 2>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
template <class CorrespondenceSearchEngineT,
          class PointToPointCorrWeightEvaluatorT = UnityWeightEvaluator<float, float>,
          class PointToPlaneCorrWeightEvaluatorT = UnityWeightEvaluator<float, float>>
using CombinedMetricRigidTransformICP3f =
    CombinedMetricSingleTransformICP<RigidTransform<float, 3>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
template <class CorrespondenceSearchEngineT,
          class PointToPointCorrWeightEvaluatorT = UnityWeightEvaluator<double, double>,
          class PointToPlaneCorrWeightEvaluatorT = UnityWeightEvaluator<double, double>>
using CombinedMetricRigidTransformICP3d =
    CombinedMetricSingleTransformICP<RigidTransform<double, 3>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
template <class CorrespondenceSearchEngineT,
          class PointToPointCorrWeightEvaluatorT = UnityWeightEvaluator<float, float>,
          class PointToPlaneCorrWeightEvaluatorT = UnityWeightEvaluator<float, float>>
using CombinedMetricAffineTransformICP2f =
    CombinedMetricSingleTransformICP<AffineTransform<float, 2>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
template <class CorrespondenceSearchEngineT,
          class PointToPointCorrWeightEvaluatorT = UnityWeightEvaluator<double, double>,
          class PointToPlaneCorrWeightEvaluatorT = UnityWeightEvaluator<double, double>>
using CombinedMetricAffineTransformICP2d =
    CombinedMetricSingleTransformICP<AffineTransform<double, 2>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
template <class CorrespondenceSearchEngineT,
          class PointToPointCorrWeightEvaluatorT = UnityWeightEvaluator<float, float>,
          class PointToPlaneCorrWeightEvaluatorT = UnityWeightEvaluator<float, float>>
using CombinedMetricAffineTransformICP3f =
    CombinedMetricSingleTransformICP<AffineTransform<float, 3>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
...
template <class CorrespondenceSearchEngineT,
          class PointToPointCorrWeightEvaluatorT = UnityWeightEvaluator<double, double>,
          class PointToPlaneCorrWeightEvaluatorT = UnityWeightEvaluator<double, double>>
using CombinedMetricAffineTransformICP3d =
    CombinedMetricSingleTransformICP<AffineTransform<double, 3>, CorrespondenceSearchEngineT,
                                     PointToPointCorrWeightEvaluatorT,
                                     PointToPlaneCorrWeightEvaluatorT>;
