...
struct DepthValueConverter {
  ...
  inline MetricDepthT getMetricValue(RawDepthT val) const {
    return inverseScale * static_cast<MetricDepthT>(val);
  }
  inline RawDepthT getRawValue(MetricDepthT val) const {
    return static_cast<RawDepthT>(scale * val);
  }
...
struct TruncatedDepthValueConverter {
  ...
  TruncatedDepthValueConverter(MetricDepthT mult, MetricDepthT thresh)
      : scale(mult), inverseScale((MetricDepthT)(1.0) / mult), maxDepth(thresh) {}
...
void depthImageToPoints(
    ...
    VectorSet<typename DepthConverterT::MetricDepth, 3>& points, bool keep_invalid = false) {
...
#pragma omp parallel for private(k)
    for (size_t y = 0; y < image_h; y++) {
      ...
      for (size_t x = 0; x < image_w; x++) {
        const MetricDepth z = depth_converter.getMetricValue(depth_data[k]);
        points.col(k++).noalias() =
            extrinsics * intrinsics_inv * Vector<MetricDepth, 3>(z * x, z * y, z);
      }
    }
  } else {
    VectorSet<MetricDepth, 3> points_tmp(3, image_w * image_h);
...
template <class DepthConverterT>
void depthImageToPointsNormals(
    const typename DepthConverterT::RawDepth* depth_data, const DepthConverterT& depth_converter,
    size_t image_w, size_t image_h,
    const Eigen::Ref<const Eigen::Matrix<typename DepthConverterT::MetricDepth, 3, 3>>& intrinsics,
    VectorSet<typename DepthConverterT::MetricDepth, 3>& points,
    VectorSet<typename DepthConverterT::MetricDepth, 3>& normals, bool keep_invalid = false) {
  using MetricDepth = typename DepthConverterT::MetricDepth;
...
#pragma omp for
      for (size_t y = 1; y < image_h - 1; y++) {
        for (size_t x = 1; x < image_w - 1; x++) {
          k = y * image_w + x;
          if (points(2, k) > (MetricDepth)0.0 && points(2, k + 1) > (MetricDepth)0.0 &&
              points(2, k - 1) > (MetricDepth)0.0 && points(2, k + image_w) > (MetricDepth)0.0 &&
              points(2, k - image_w) > (MetricDepth)0.0) {
            normals.col(k).noalias() = (points.col(k + image_w) - points.col(k - image_w))
                                           .cross(points.col(k + 1) - points.col(k - 1))
                                           .normalized();
          }
        }
      }
    ...
    VectorSet<MetricDepth, 3> points_tmp(3, image_w * image_h);
    VectorSet<MetricDepth, 3> normals_tmp(VectorSet<MetricDepth, 3>::Constant(
        3, points_tmp.cols(), std::numeric_limits<MetricDepth>::quiet_NaN()));
    size_t valid_count = 0;
...
#pragma omp for reduction(+ : valid_count)
      for (size_t y = 1; y < image_h - 1; y++) {
        for (size_t x = 1; x < image_w - 1; x++) {
          k = y * image_w + x;
          if (points_tmp(2, k) > (MetricDepth)0.0 && points_tmp(2, k + 1) > (MetricDepth)0.0 &&
              points_tmp(2, k - 1) > (MetricDepth)0.0 &&
              points_tmp(2, k + image_w) > (MetricDepth)0.0 &&
              points_tmp(2, k - image_w) > (MetricDepth)0.0) {
            normals_tmp.col(k).noalias() =
                (points_tmp.col(k + image_w) - points_tmp.col(k - image_w))
                    .cross(points_tmp.col(k + 1) - points_tmp.col(k - 1))
                    .normalized();
            valid_count += 1;
          }
        }
...
template <class DepthConverterT>
void RGBDImagesToPointsColors(
    const unsigned char* rgb_data, const typename DepthConverterT::RawDepth* depth_data,
    const DepthConverterT& depth_converter, size_t image_w, size_t image_h,
    const Eigen::Ref<const Eigen::Matrix<typename DepthConverterT::MetricDepth, 3, 3>>& intrinsics,
    VectorSet<typename DepthConverterT::MetricDepth, 3>& points, VectorSet<float, 3>& colors,
...
template <class DepthConverterT>
void RGBDImagesToPointsNormalsColors(
    const unsigned char* rgb_data, const typename DepthConverterT::RawDepth* depth_data,
    const DepthConverterT& depth_converter, size_t image_w, size_t image_h,
    const Eigen::Ref<const Eigen::Matrix<typename DepthConverterT::MetricDepth, 3, 3>>& intrinsics,
    VectorSet<typename DepthConverterT::MetricDepth, 3>& points,
    VectorSet<typename DepthConverterT::MetricDepth, 3>& normals, VectorSet<float, 3>& colors,
    bool keep_invalid = false) {
  using MetricDepth = typename DepthConverterT::MetricDepth;
...
template <class DepthConverterT>
void pointsToDepthImage(
    const ConstVectorSetMatrixMap<typename DepthConverterT::MetricDepth, 3>& points,
    const Eigen::Ref<const Eigen::Matrix<typename DepthConverterT::MetricDepth, 3, 3>>& intrinsics,
    const DepthConverterT& depth_converter, typename DepthConverterT::RawDepth* depth_data,
    size_t image_w, size_t image_h) {
  using MetricDepth = typename DepthConverterT::MetricDepth;
...
template <class DepthConverterT>
void pointsColorsToRGBDImages(
    const ConstVectorSetMatrixMap<typename DepthConverterT::MetricDepth, 3>& points,
    const ConstVectorSetMatrixMap<float, 3>& colors,
    const Eigen::Ref<const Eigen::Matrix<typename DepthConverterT::MetricDepth, 3, 3>>& intrinsics,
    const DepthConverterT& depth_converter, unsigned char* rgb_data,
    typename DepthConverterT::RawDepth* depth_data, size_t image_w, size_t image_h) {
  using MetricDepth = typename DepthConverterT::MetricDepth;
  using RawDepth = typename DepthConverterT::RawDepth;
...
void pointsToIndexMap(const ConstVectorSetMatrixMap<PointT, 3>& points,
                      ...
                      IndexT* index_map_data, size_t image_w, size_t image_h) {
