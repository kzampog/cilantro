...
struct EigenMap {
  ...
  EigenMap(const Eigen::Map<const Eigen::Matrix<ScalarT, EigenDim, Eigen::Dynamic>>& obj_)
      : obj(obj_) {}
...
using L1 = nanoflann::L1_Adaptor<typename DataAdaptor::Scalar, DataAdaptor,
                                 typename DataAdaptor::Scalar, typename DataAdaptor::Index>;

template <class DataAdaptor>
using L2 = nanoflann::L2_Adaptor<typename DataAdaptor::Scalar, DataAdaptor,
                                 typename DataAdaptor::Scalar, typename DataAdaptor::Index>;

template <class DataAdaptor>
using L2Simple =
    nanoflann::L2_Simple_Adaptor<typename DataAdaptor::Scalar, DataAdaptor,
                                 typename DataAdaptor::Scalar, typename DataAdaptor::Index>;

template <class DataAdaptor>
using SO2 = nanoflann::SO2_Adaptor<typename DataAdaptor::Scalar, DataAdaptor,
                                   typename DataAdaptor::Scalar, typename DataAdaptor::Index>;

template <class DataAdaptor>
using SO3 = nanoflann::SO3_Adaptor<typename DataAdaptor::Scalar, DataAdaptor,
                                   typename DataAdaptor::Scalar, typename DataAdaptor::Index>;
}  // namespace KDTreeDistanceAdaptors
...
public:
  ...
  inline const KDTree& nearestNeighborSearch(
      const Eigen::Ref<const Vector<ScalarT, EigenDim>>& query_pt, NeighborResult& result) const {
    kd_tree_.knnSearch(query_pt.data(), 1, &result.index, &result.value);
    return *this;
  }

  // Do not call if tree is empty!
  inline NeighborResult nearestNeighborSearch(
      const Eigen::Ref<const Vector<ScalarT, EigenDim>>& query_pt) const {
...
#pragma omp parallel for shared(results)
  ...
  inline NeighborhoodResult nearestNeighborSearch(
      const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts) const {
  ...
  template <typename CountT = size_t>
  const KDTree& kNNSearch(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts, CountT k,
...
#pragma omp parallel for shared(results)
  ...
  template <typename CountT = size_t>
  inline NeighborhoodSetResult kNNSearch(
      const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts, CountT k) const {
  ...
  inline NeighborhoodResult radiusSearch(
      const Eigen::Ref<const Vector<ScalarT, EigenDim>>& query_pt, ScalarT radius) const {
    NeighborhoodResult results;
  ...
  const KDTree& radiusSearch(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts,
                             ScalarT radius, NeighborhoodSetResult& results) const {
    results.resize(query_pts.cols());
#pragma omp parallel for shared(results)
    for (size_t i = 0; i < query_pts.cols(); i++) {
  ...
  inline NeighborhoodSetResult radiusSearch(
      const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts, ScalarT radius) const {
    NeighborhoodSetResult results;
  ...
  template <typename CountT = size_t>
  inline const KDTree& kNNInRadiusSearch(
  ...
  template <typename CountT = size_t>
  inline NeighborhoodResult kNNInRadiusSearch(
      const Eigen::Ref<const Vector<ScalarT, EigenDim>>& query_pt, CountT k, ScalarT radius) const {
    NeighborhoodResult results;
  ...
  const KDTree& kNNInRadiusSearch(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts,
                                  CountT k, ScalarT radius, NeighborhoodSetResult& results) const {
    results.resize(query_pts.cols());
#pragma omp parallel for shared(results)
    for (size_t i = 0; i < query_pts.cols(); i++) {
  ...
  template <typename CountT = size_t>
  inline NeighborhoodSetResult kNNInRadiusSearch(
      const ConstVectorSetMatrixMap<ScalarT, EigenDim>& query_pts, CountT k, ScalarT radius) const {
    NeighborhoodSetResult results;
  ...
  inline typename std::enable_if<PointT::ColsAtCompileTime == 1, NeighborhoodResult>::type search(
      const PointT& query_pt, const NeighborhoodSpecT& nh) const {
  ...
  inline typename std::enable_if<PointsT::ColsAtCompileTime == Eigen::Dynamic,
                                 NeighborhoodSetResult>::type
  search(const PointsT& query_pts, const NeighborhoodSpecT& nh) const {
