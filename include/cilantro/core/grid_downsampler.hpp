...
public:
  ...
  const PointsGridDownsampler& getDownsampledPoints(VectorSet<ScalarT, EigenDim>& ds_points,
...
template <typename ScalarT, ptrdiff_t EigenDim, typename GridPointScalarT = ptrdiff_t>
class PointsNormalsGridDownsampler
...
public:
  ...
  PointsNormalsGridDownsampler(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points,
      ...
      : Base(points, bin_size, PointNormalSumAccumulatorProxy<ScalarT, EigenDim>(points, normals),
             parallel) {}

  const PointsNormalsGridDownsampler& getDownsampledPoints(VectorSet<ScalarT, EigenDim>& ds_points,
                                                           size_t min_points_in_bin = 1) const {
    ds_points.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
  ...
  const PointsNormalsGridDownsampler& getDownsampledNormals(
      VectorSet<ScalarT, EigenDim>& ds_normals, size_t min_points_in_bin = 1) const {
  ...
  const PointsNormalsGridDownsampler& getDownsampledPointsNormals(
      VectorSet<ScalarT, EigenDim>& ds_points, VectorSet<ScalarT, EigenDim>& ds_normals,
      size_t min_points_in_bin = 1) const {
    ds_points.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
    ds_normals.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
...
template <typename ScalarT, ptrdiff_t EigenDim, typename GridPointScalarT = ptrdiff_t>
class PointsColorsGridDownsampler
...
public:
  ...
  PointsColorsGridDownsampler(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points,
                              ...
                              bool parallel = true)
      : Base(points, bin_size, PointColorSumAccumulatorProxy<ScalarT, EigenDim>(points, colors),
             parallel) {}

  const PointsColorsGridDownsampler& getDownsampledPoints(VectorSet<ScalarT, EigenDim>& ds_points,
                                                          size_t min_points_in_bin = 1) const {
    ds_points.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
  ...
  const PointsColorsGridDownsampler& getDownsampledColors(VectorSet<float, 3>& ds_colors,
                                                          size_t min_points_in_bin = 1) const {
  ...
  const PointsColorsGridDownsampler& getDownsampledPointsColors(
      VectorSet<ScalarT, EigenDim>& ds_points, VectorSet<float, 3>& ds_colors,
      size_t min_points_in_bin = 1) const {
    ds_points.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
    ds_colors.resize(3, this->grid_lookup_table_.size());
...
template <typename ScalarT, ptrdiff_t EigenDim, typename GridPointScalarT = ptrdiff_t>
class PointsNormalsColorsGridDownsampler
    : public GridAccumulator<ScalarT, EigenDim,
...
public:
  ...
  PointsNormalsColorsGridDownsampler(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points,
                                     ...
                                     ScalarT bin_size, bool parallel = true)
      : Base(points, bin_size,
             PointNormalColorSumAccumulatorProxy<ScalarT, EigenDim>(points, normals, colors),
             parallel) {}

  const PointsNormalsColorsGridDownsampler& getDownsampledPoints(
      VectorSet<ScalarT, EigenDim>& ds_points, size_t min_points_in_bin = 1) const {
    ds_points.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
  ...
  const PointsNormalsColorsGridDownsampler& getDownsampledNormals(
      VectorSet<ScalarT, EigenDim>& ds_normals, size_t min_points_in_bin = 1) const {
    ds_normals.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
  ...
  const PointsNormalsColorsGridDownsampler& getDownsampledColors(
      VectorSet<float, 3>& ds_colors, size_t min_points_in_bin = 1) const {
    ds_colors.resize(3, this->grid_lookup_table_.size());
  ...
  const PointsNormalsColorsGridDownsampler& getDownsampledPointsNormalsColors(
      VectorSet<ScalarT, EigenDim>& ds_points, VectorSet<ScalarT, EigenDim>& ds_normals,
      VectorSet<float, 3>& ds_colors, size_t min_points_in_bin = 1) const {
    ds_points.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
    ds_normals.resize(this->data_map_.rows(), this->grid_lookup_table_.size());
...
typedef PointsNormalsColorsGridDownsampler<float, Eigen::Dynamic>
    PointsNormalsColorsGridDownsamplerXf;
typedef PointsNormalsColorsGridDownsampler<double, Eigen::Dynamic>
    PointsNormalsColorsGridDownsamplerXd;
