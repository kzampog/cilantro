...
template <typename ScalarT, ptrdiff_t EigenDim, class AccumulatorProxyT,
...
public:
  ...
  GridAccumulator(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& data,
                  ...
                  const AccumulatorProxy& accum_proxy, bool parallel = true)
      : data_map_(data), bin_size_(bin_size), bin_size_inv_(bin_size_.cwiseInverse()) {
    build_index_(accum_proxy, parallel);
  }

  GridAccumulator(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& data, ScalarT bin_size,
                  const AccumulatorProxy& accum_proxy, bool parallel = true)
      : data_map_(data),
  ...
  inline const std::vector<GridBinMapIterator>& getOccupiedBinIterators() const {
    return bin_iterators_;
  }

  inline const GridBinMapConstIterator findContainingGridBin(
      const Eigen::Ref<const Vector<ScalarT, EigenDim>>& point) const {
  ...
  inline GridPoint getPointGridCoordinates(
      const Eigen::Ref<const Vector<ScalarT, EigenDim>>& point) const {
    // return point.cwiseProduct(bin_size_inv_).array().floor().template cast<typename
    // GridPoint::Scalar>();
  ...
  inline Vector<ScalarT, EigenDim> getBinCornerCoordinates(
      const Eigen::Ref<const GridPoint>& grid_point) const {
...
#pragma omp critical
        {
          for (auto it = lookup_priv.begin(); it != lookup_priv.end(); ++it) {
            ...
            if (lb != grid_lookup_table_.end() &&
                !(grid_lookup_table_.key_comp()(it->first, lb->first))) {
      ...
      for (size_t i = 0; i < data_map_.cols(); i++) {
        ...
        if (lb != grid_lookup_table_.end() &&
            !(grid_lookup_table_.key_comp()(grid_coords, lb->first))) {
          accum_proxy.addToAccumulator(lb->second, i);
        } else {
          bin_iterators_.emplace_back(grid_lookup_table_.emplace_hint(
              lb, std::move(grid_coords), accum_proxy.buildAccumulator(i)));
        }
      }
    }
  }
