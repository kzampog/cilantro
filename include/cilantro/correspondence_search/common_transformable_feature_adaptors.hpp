...
#pragma omp parallel for
  ...
  inline const ConstVectorSetMatrixMap<ScalarT, FeatureDimension>& getTransformedFeaturesMatrixMap()
      const {
...
#pragma omp parallel for
      ...
      const ScalarT normal_weight =
          (data_map_.cols() > 0) ? data_map_.template block<3, 1>(dim, 0).norm() : (ScalarT)0.0;
#pragma omp parallel for
      for (size_t i = 0; i < data_map_.cols(); i++) {
        auto res_col = transformed_data_.col(i);
        auto data_col = data_map_.col(i);
        res_col.head(dim).noalias() = tform.linear() * data_col.head(dim) + tform.translation();
        res_col.tail(dim).noalias() =
            normal_weight *
            (tform.linear().inverse().transpose() * data_col.tail(dim)).normalized();
      }
    }
    return *this;
  }
...
#pragma omp parallel for
      for (size_t i = 0; i < data_map_.cols(); i++) {
        auto res_col = transformed_data_.col(i);
        auto data_col = data_map_.col(i);
        res_col.head(dim).noalias() =
            tforms[i].linear() * data_col.head(dim) + tforms[i].translation();
...
#pragma omp parallel for
      for (size_t i = 0; i < data_map_.cols(); i++) {
        ...
        res_col.tail(dim).noalias() =
            normal_weight *
            (tforms[i].linear().inverse().transpose() * data_col.tail(dim)).normalized();
      }
    }
    return *this;
  }
...
public:
  ...
  PointColorFeaturesAdaptor(const ConstVectorSetMatrixMap<ScalarT, EigenDim>& points,
                            const ConstVectorSetMatrixMap<float, 3>& colors, ScalarT color_weight)
...
#pragma omp parallel for
      for (size_t i = 0; i < data_map_.cols(); i++) {
        ...
        auto data_col = data_map_.col(i);
        res_col.head(dim).noalias() = tform.linear() * data_col.head(dim) + tform.translation();
        res_col.segment(dim, dim).noalias() =
            normal_weight *
            (tform.linear().inverse().transpose() * data_col.segment(dim, dim)).normalized();
        res_col.tail(3) = data_col.tail(3);
      }
    }
    return *this;
...
#pragma omp parallel for
      for (size_t i = 0; i < data_map_.cols(); i++) {
        ...
        res_col.segment(dim, dim).noalias() =
            normal_weight *
            (tforms[i].linear().inverse().transpose() * data_col.segment(dim, dim)).normalized();
        res_col.tail(3) = data_col.tail(3);
      }
    }
    return *this;
