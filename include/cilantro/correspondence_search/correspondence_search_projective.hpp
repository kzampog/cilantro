...
template <class ScalarT, class EvaluationFeatureAdaptorT = PointFeaturesAdaptor<ScalarT, 3>,
...
public:
  ...
  template <class EvalFeatAdaptorT = EvaluationFeatureAdaptorT,
            class = typename std::enable_if<
                std::is_same<EvalFeatAdaptorT, PointFeaturesAdaptor<ScalarT, 3>>::value>::type>
  CorrespondenceSearchProjective(PointFeaturesAdaptor<ScalarT, 3>& dst_points,
      ...
      : dst_search_features_adaptor_(dst_points),
        src_search_features_adaptor_(src_points),
        src_evaluation_features_adaptor_(src_points),
        evaluator_(evaluator),
        projection_image_width_(640),
        projection_image_height_(480),
  ...
  inline CorrespondenceSearchProjective& findCorrespondences(const TransformT& tform) {
    if (!std::is_same<PointFeaturesAdaptor<ScalarT, 3>, EvaluationFeatureAdaptorT>::value ||
        &src_search_features_adaptor_ !=
            (PointFeaturesAdaptor<ScalarT, 3>*)(&src_evaluation_features_adaptor_)) {
  ...
  inline const Eigen::Matrix<ScalarT, 3, 3>& getProjectionIntrinsicMatrix() const {
    return projection_intrinsics_;
  }

  inline CorrespondenceSearchProjective& setProjectionIntrinsicMatrix(
      const Eigen::Ref<const Eigen::Matrix<ScalarT, 3, 3>>& mat) {
  ...
  inline CorrespondenceSearchProjective& setProjectionExtrinsicMatrix(
      const RigidTransform<ScalarT, 3>& mat) {
...
private:
  ...
  void find_correspondences_(const ConstVectorSetMatrixMap<ScalarT, 3>& src_points_trans,
                             SearchResult& correspondences) {
    const ConstVectorSetMatrixMap<ScalarT, 3>& dst_points(
        dst_search_features_adaptor_.getFeaturesMatrixMap());

    if (index_map_.rows() != projection_image_width_ ||
        index_map_.cols() != projection_image_height_) {
      index_map_.resize(projection_image_width_, projection_image_height_);
      pointsToIndexMap<ScalarT, IndexT>(dst_points, projection_extrinsics_, projection_intrinsics_,
                                        index_map_.data(), projection_image_width_,
                                        projection_image_height_);
    }
...
#pragma omp for schedule(dynamic, 256)
      for (size_t i = 0; i < src_points_trans.cols(); i++) {
        ...
        corr_tmp[i].value =
            evaluator_(ind, i, (src_points_trans.col(i) - dst_points.col(ind)).squaredNorm());
      }
    }
