...
public:
  ...
  ModelEstimatorT& estimate() {
    ...
    if (re_estimate_) {
      ...
      size_t k = 0;
      for (size_t i = 0; i < num_points; i++) {
        if (model_residuals_[i] <= inlier_dist_thresh_)
          model_inliers_[k++] = static_cast<IndexT>(i);
      }
  ...
  inline ModelEstimatorT& estimate(ResidualScalar max_residual, size_t target_inlier_count,
                                   size_t max_iter) {
