...
class TransformRANSACEstimator
    : public RandomSampleConsensusBase<TransformRANSACEstimator<TransformT>, TransformT,
                                       typename TransformT::Scalar, IndexT> {
  typedef RandomSampleConsensusBase<TransformRANSACEstimator<TransformT>, TransformT,
                                    typename TransformT::Scalar, IndexT>
      Base;
...
public:
  ...
  TransformRANSACEstimator(const ConstVectorSetMatrixMap<Scalar, Dim>& dst_points,
                           const ConstVectorSetMatrixMap<Scalar, Dim>& src_points,
                           const std::vector<IdxT>& dst_ind, const std::vector<IdxT>& src_ind)
...
template <typename IndexT = size_t>
using AffineTransformRANSACEstimator2f =
    TransformRANSACEstimator<AffineTransform<float, 2>, IndexT>;

template <typename IndexT = size_t>
using AffineTransformRANSACEstimator2d =
    TransformRANSACEstimator<AffineTransform<double, 2>, IndexT>;

template <typename IndexT = size_t>
using AffineTransformRANSACEstimator3f =
    TransformRANSACEstimator<AffineTransform<float, 3>, IndexT>;

template <typename IndexT = size_t>
using AffineTransformRANSACEstimator3d =
    TransformRANSACEstimator<AffineTransform<double, 3>, IndexT>;
