...
template <class Matrix>
bool writeEigenMatrixToFile(const std::string& file_path, const Matrix& matrix,
                            bool binary = true) {
  if (binary) {
...
inline bool fileExists(const std::string& file_path) { return std::ifstream(file_path).is_open(); }
...
inline size_t readRawDataFromFile(const std::string& file_path, void* data_ptr,
                                  size_t num_bytes = 0) {
...
inline bool writeRawDataToFile(const std::string& file_path, const void* data_ptr,
                               size_t num_bytes) {
