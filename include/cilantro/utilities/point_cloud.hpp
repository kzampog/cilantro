...
public:
  ...
  PointCloud(const PointCloud<ScalarT, EigenDim>& cloud, const std::vector<IndexT>& indices,
             bool negate = false) {
    std::set<IndexT> indices_set;
    if (negate) {
      std::vector<IndexT> full_indices(cloud.size());
      for (size_t i = 0; i < cloud.size(); i++) full_indices[i] = static_cast<IndexT>(i);
      std::set<IndexT> indices_to_discard(indices.begin(), indices.end());
      std::set_difference(full_indices.begin(), full_indices.end(), indices_to_discard.begin(),
                          indices_to_discard.end(),
                          std::inserter(indices_set, indices_set.begin()));
    } else {
  ...
  template <class DepthConverterT,
            class = typename std::enable_if<
                EigenDim == 3 &&
                std::is_same<typename DepthConverterT::MetricDepth, ScalarT>::value>::type>
  PointCloud(const typename DepthConverterT::RawDepth* depth_data,
             const DepthConverterT& depth_converter, size_t image_w, size_t image_h,
             const Eigen::Ref<const Eigen::Matrix<ScalarT, 3, 3>>& intrinsics,
             bool keep_invalid = false, bool compute_normals = false) {
    if (compute_normals) {
      depthImageToPointsNormals<DepthConverterT>(depth_data, depth_converter, image_w, image_h,
                                                 intrinsics, points, normals, keep_invalid);
    } else {
      depthImageToPoints<DepthConverterT>(depth_data, depth_converter, image_w, image_h, intrinsics,
                                          points, keep_invalid);
    }
  }
  ...
  template <class DepthConverterT,
            class = typename std::enable_if<
                EigenDim == 3 &&
                std::is_same<typename DepthConverterT::MetricDepth, ScalarT>::value>::type>
  PointCloud(unsigned char* rgb_data, const typename DepthConverterT::RawDepth* depth_data,
             const DepthConverterT& depth_converter, size_t image_w, size_t image_h,
             const Eigen::Ref<const Eigen::Matrix<ScalarT, 3, 3>>& intrinsics,
             bool keep_invalid = false, bool compute_normals = false) {
    if (compute_normals) {
      RGBDImagesToPointsNormalsColors<DepthConverterT>(rgb_data, depth_data, depth_converter,
                                                       image_w, image_h, intrinsics, points,
                                                       normals, colors, keep_invalid);
    } else {
      RGBDImagesToPointsColors<DepthConverterT>(rgb_data, depth_data, depth_converter, image_w,
                                                image_h, intrinsics, points, colors, keep_invalid);
    }
  }
  ...
  PointCloud& removeInvalidData() {
    ...
    ind_to_remove.reserve(points.cols());
    for (size_t i = 0; i < points.cols(); i++) {
      if (!points.col(i).allFinite() || (has_normals && !normals.col(i).allFinite()) ||
          (has_colors && !colors.col(i).allFinite())) {
  ...
  PointCloud& gridDownsample(ScalarT bin_size, size_t min_points_in_bin = 1, bool parallel = true) {
    if (hasNormals() && hasColors()) {
      PointsNormalsColorsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(
          points, normals, colors, bin_size, parallel)
          .getDownsampledPointsNormalsColors(points, normals, colors, min_points_in_bin);
    } else if (hasNormals()) {
      PointsNormalsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(points, normals, bin_size,
                                                                        parallel)
          .getDownsampledPointsNormals(points, normals, min_points_in_bin);
    } else if (hasColors()) {
      PointsColorsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(points, colors, bin_size,
                                                                       parallel)
          .getDownsampledPointsColors(points, colors, min_points_in_bin);
    } else {
      PointsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(points, bin_size, parallel)
          .getDownsampledPoints(points, min_points_in_bin);
    }
    return *this;
  }
  ...
  PointCloud gridDownsampled(ScalarT bin_size, size_t min_points_in_bin = 1,
    ...
    if (hasNormals() && hasColors()) {
      PointsNormalsColorsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(
          points, normals, colors, bin_size, parallel)
          .getDownsampledPointsNormalsColors(res.points, res.normals, res.colors,
                                             min_points_in_bin);
    } else if (hasNormals()) {
      PointsNormalsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(points, normals, bin_size,
                                                                        parallel)
          .getDownsampledPointsNormals(res.points, res.normals, min_points_in_bin);
    } else if (hasColors()) {
      PointsColorsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(points, colors, bin_size,
                                                                       parallel)
          .getDownsampledPointsColors(res.points, res.colors, min_points_in_bin);
    } else {
      PointsGridDownsampler<ScalarT, EigenDim, GridPointScalarT>(points, bin_size, parallel)
          .getDownsampledPoints(res.points, min_points_in_bin);
    }
    return res;
  }

  template <typename CovarianceT = Covariance<ScalarT, EigenDim>, typename IndexT = size_t,
            typename CountT = size_t>
  inline PointCloud& estimateNormalsKNN(CountT k, bool use_current_as_ref = false) {
    ...
    if (use_current_as_ref) {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(points)
          .setReferenceNormals(normals)
          .estimateNormalsKNN(normals, k);
    } else {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(points)
          .setViewPoint(Vector<ScalarT, EigenDim>::Zero(points.rows(), 1))
          .estimateNormalsKNN(normals, k);
    }
    return *this;
  }
  ...
  template <typename CovarianceT = Covariance<ScalarT, EigenDim>, typename IndexT = size_t,
            typename CountT = size_t>
  inline PointCloud& estimateNormalsKNN(
      const KDTree<ScalarT, EigenDim, KDTreeDistanceAdaptors::L2, IndexT>& kd_tree, CountT k,
      bool use_current_as_ref = false) {
    use_current_as_ref = use_current_as_ref && hasNormals();
    normals.resize(points.rows(), points.cols());
    if (use_current_as_ref) {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(kd_tree)
          .setReferenceNormals(normals)
          .estimateNormalsKNN(normals, k);
    } else {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(kd_tree)
  ...
  inline PointCloud& estimateNormalsRadius(ScalarT radius, bool use_current_as_ref = false) {
    ...
    if (use_current_as_ref) {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(points)
          .setReferenceNormals(normals)
          .estimateNormalsRadius(normals, radius);
    } else {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(points)
          .setViewPoint(Vector<ScalarT, EigenDim>::Zero(points.rows(), 1))
          .estimateNormalsRadius(normals, radius);
  ...
  template <typename CovarianceT = Covariance<ScalarT, EigenDim>, typename IndexT = size_t>
  inline PointCloud& estimateNormalsRadius(
      const KDTree<ScalarT, EigenDim, KDTreeDistanceAdaptors::L2, IndexT>& kd_tree, ScalarT radius,
      bool use_current_as_ref = false) {
    use_current_as_ref = use_current_as_ref && hasNormals();
    normals.resize(points.rows(), points.cols());
    if (use_current_as_ref) {
  ...
  template <typename CovarianceT = Covariance<ScalarT, EigenDim>, typename IndexT = size_t,
            typename CountT = size_t>
  inline PointCloud& estimateNormalsKNNInRadius(CountT k, ScalarT radius,
                                                bool use_current_as_ref = false) {
    use_current_as_ref = use_current_as_ref && hasNormals();
    normals.resize(points.rows(), points.cols());
    if (use_current_as_ref) {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(points)
          .setReferenceNormals(normals)
          .estimateNormalsKNNInRadius(normals, k, radius);
    } else {
      NormalEstimation<ScalarT, EigenDim, CovarianceT, IndexT>(points)
          .setViewPoint(Vector<ScalarT, EigenDim>::Zero(points.rows(), 1))
          .estimateNormalsKNNInRadius(normals, k, radius);
  ...
  template <typename CovarianceT = Covariance<ScalarT, EigenDim>, typename IndexT = size_t,
            typename CountT = size_t>
  inline PointCloud& estimateNormalsKNNInRadius(
      const KDTree<ScalarT, EigenDim, KDTreeDistanceAdaptors::L2, IndexT>& kd_tree, CountT k,
  ...
  template <typename NeighborhoodSpecT, typename CovarianceT = Covariance<ScalarT, EigenDim>,
  ...
  template <typename NeighborhoodSpecT, typename CovarianceT = Covariance<ScalarT, EigenDim>,
            typename IndexT = size_t>
  inline PointCloud& estimateNormals(
      const KDTree<ScalarT, EigenDim, KDTreeDistanceAdaptors::L2, IndexT>& kd_tree,
  ...
  template <class DepthConverterT,
            class = typename std::enable_if<
                EigenDim == 3 &&
                std::is_same<typename DepthConverterT::MetricDepth, ScalarT>::value>::type>
  inline PointCloud& fromDepthImage(
      const typename DepthConverterT::RawDepth* depth_data, const DepthConverterT& depth_converter,
      size_t image_w, size_t image_h,
      const Eigen::Ref<const Eigen::Matrix<ScalarT, 3, 3>>& intrinsics, bool keep_invalid = false,
  ...
  template <class DepthConverterT,
            class = typename std::enable_if<
                EigenDim == 3 &&
                std::is_same<typename DepthConverterT::MetricDepth, ScalarT>::value>::type>
  inline PointCloud& fromRGBDImages(
      unsigned char* rgb_data, const typename DepthConverterT::RawDepth* depth_data,
      const DepthConverterT& depth_converter, size_t image_w, size_t image_h,
      const Eigen::Ref<const Eigen::Matrix<ScalarT, 3, 3>>& intrinsics, bool keep_invalid = false,
      bool compute_normals = false) {
    if (compute_normals) {
  ...
  template <typename ScalarOutT = ScalarT, ptrdiff_t Dim = EigenDim,
