...
public:
  inline PLYReader(const std::string& file_path, bool preload = true) {
    if (preload) {
      ...
      input_stream_.reset(
          new internal::MemoryStream((char*)preloaded_data_.data(), preloaded_data_.size()));
    } else {
  ...
  inline std::shared_ptr<tinyply::PlyData> requestData(
      const std::string& element_key, const std::vector<std::string>& property_keys,
      const uint32_t list_size_hint = 0) {
    if (!elementPropertyExists(element_key, property_keys))
      return std::shared_ptr<tinyply::PlyData>();
...
public:
  ...
  inline PLYWriter& addData(const std::string& element_key,
    ...
    ply_file_.add_properties_to_element(element_key, property_keys, data_buffer->t,
                                        data_buffer->count, data_buffer->buffer.get(), list_type,
                                        list_count);
    return *this;
  }
...
VectorSet<ScalarT, EigenDim> vectorSetFromPLYDataBuffer(
    const std::shared_ptr<tinyply::PlyData>& data_buffer, size_t dim) {
  ...
  case tinyply::Type::INT8:
    data_matrix = ConstVectorSetMatrixMap<int8_t, EigenDim>((int8_t*)data_buffer->buffer.get(), dim,
                                                            data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::UINT8:
    data_matrix = ConstVectorSetMatrixMap<uint8_t, EigenDim>((uint8_t*)data_buffer->buffer.get(),
                                                             dim, data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::INT16:
    data_matrix = ConstVectorSetMatrixMap<int16_t, EigenDim>((int16_t*)data_buffer->buffer.get(),
                                                             dim, data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::UINT16:
    data_matrix = ConstVectorSetMatrixMap<uint16_t, EigenDim>((uint16_t*)data_buffer->buffer.get(),
                                                              dim, data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::INT32:
    data_matrix = ConstVectorSetMatrixMap<int32_t, EigenDim>((int32_t*)data_buffer->buffer.get(),
                                                             dim, data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::UINT32:
    data_matrix = ConstVectorSetMatrixMap<uint32_t, EigenDim>((uint32_t*)data_buffer->buffer.get(),
                                                              dim, data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::FLOAT32:
    data_matrix = ConstVectorSetMatrixMap<float, EigenDim>((float*)data_buffer->buffer.get(), dim,
                                                           data_buffer->count)
                      .template cast<ScalarT>();
    break;
  case tinyply::Type::FLOAT64:
    data_matrix = ConstVectorSetMatrixMap<double, EigenDim>((double*)data_buffer->buffer.get(), dim,
                                                            data_buffer->count)
                      .template cast<ScalarT>();
    break;
  default:
...
std::shared_ptr<tinyply::PlyData> PLYDataBufferFromVectorSet(
    const ConstVectorSetMatrixMap<ScalarT, EigenDim>& data_matrix) {
  ...
  } else if constexpr (std::is_same<ScalarOutT, uint8_t>::value ||
                       std::is_same<ScalarOutT, unsigned char>::value) {
    data_buffer->t = tinyply::Type::UINT8;
  } else if constexpr (std::is_same<ScalarOutT, int16_t>::value ||
                       std::is_same<ScalarOutT, short>::value) {
    data_buffer->t = tinyply::Type::INT16;
  } else if constexpr (std::is_same<ScalarOutT, uint16_t>::value ||
                       std::is_same<ScalarOutT, unsigned short>::value) {
    data_buffer->t = tinyply::Type::UINT16;
  } else if constexpr (std::is_same<ScalarOutT, int32_t>::value ||
                       std::is_same<ScalarOutT, int>::value) {
    data_buffer->t = tinyply::Type::INT32;
  } else if constexpr (std::is_same<ScalarOutT, uint32_t>::value ||
                       std::is_same<ScalarOutT, unsigned int>::value) {
  ...
  if constexpr (std::is_same<ScalarT, ScalarOutT>::value) {
    ...
    data_buffer->buffer =
        tinyply::Buffer(data_matrix.rows() * data_matrix.cols() * sizeof(ScalarOutT));
    DataMatrixMap<ScalarOutT, EigenDim>((ScalarOutT*)data_buffer->buffer.get(), data_matrix.rows(),
                                        data_matrix.cols())
        .base() = data_matrix.template cast<ScalarOutT>();
  }
