...
VectorSet<float, 3> colormap(const ConstVectorSetMatrixMap<ValueT, 1>& scalars,
  ...
  const ValueT scalar_range =
      (scalar_max_used == scalar_min_used) ? (ValueT)1.0 : (scalar_max_used - scalar_min_used);
...
#pragma omp parallel for
    for (size_t i = 0; i < scalars.cols(); i++) {
      colors.col(i).setConstant(
          std::max(std::min((scalars[i] - scalar_min_used) / scalar_range, 1.0f), 0.0f));
    }
    break;
