...
void generate_input_data(cilantro::PointCloud3f& dst, cilantro::PointCloud3f& src,
...
int main(int argc, char** argv) {
  ...
  // // cilantro::PointNormalColorFeaturesAdaptor3f dst_feat(
  // //     dst.points, dst.normals, dst.colors, 0.5, 5.0);
  // // cilantro::PointNormalColorFeaturesAdaptor3f src_feat(
  // //     src.points, src.normals, src.colors, 0.5, 5.0);
  ...
  // cilantro::CorrespondenceSearchKDTree<decltype(dst_feat)> corr_engine(
  //     dst_feat, src_feat, dist_eval);

  // cilantro::UnityWeightEvaluator<float> corr_weight_eval;

  // // Point-to-point
  // // cilantro::PointToPointMetricRigidTransformICP3f<decltype(corr_engine)> icp(
  // //     dst.points, src.points, corr_engine);
  // // Weighted combination of point-to-point and point-to-plane
  // cilantro::CombinedMetricRigidTransformICP3f<decltype(corr_engine)> icp(
  //     dst.points, dst.normals, src.points, corr_engine, corr_weight_eval, corr_weight_eval);
  ...
  // Parameter setting
  icp.setMaxNumberOfOptimizationStepIterations(1)
      .setPointToPointMetricWeight(0.0f)
  ...
  initial_viz.addObject<cilantro::PointCloudRenderable>(
  ...
  registration_viz.addObject<cilantro::PointCloudRenderable>(
      "dst", dst, cilantro::RenderingProperties().setPointColor(0, 0, 1));
  registration_viz.addObject<cilantro::PointCloudRenderable>(
      "src", src_t, cilantro::RenderingProperties().setPointColor(1, 0, 0));

  // Residuals
  residuals_viz
      .addObject<cilantro::PointCloudRenderable>(
          "src", src_t, cilantro::RenderingProperties().setUseLighting(false))
      ->setPointValues(residuals);

  std::cout << "Press 'c' to toggle point cloud colors" << std::endl;
  while (!initial_viz.wasStopped() && !registration_viz.wasStopped() &&
