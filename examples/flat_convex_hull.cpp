...
int main(int argc, char** argv) {
  ...
  viz.addObject<cilantro::PointCloudRenderable>("cloud", cloud,
                                                cilantro::RenderingProperties().setOpacity(0.5));
  viz.addObject<cilantro::PointCloudRenderable>(
      "hull_cloud", flat_hull.getVertices3D(),
      cilantro::RenderingProperties().setPointColor(1, 0, 0).setPointSize(10.0));
  viz.addObject<cilantro::PointCorrespondencesRenderable>("hull_lines", src_points, dst_points,
                                                          cilantro::RenderingProperties()
                                                              .setLineColor(0, 0, 1)
                                                              .setLineWidth(5.0)
                                                              .setLineDensityFraction(1.0));
  viz.spin();
