...
void generate_input_data(cilantro::VectorSet3f& original_points,
                         cilantro::VectorSet<float, 1>& values, Eigen::MatrixXf& dist_sq) {
...
int main(int argc, char** argv) {
  ...
  viz1.addObject<cilantro::PointCloudRenderable>("cloud", points,
                                                 cilantro::RenderingProperties().setPointSize(3.0f))
      ->setPointValues(values);

  cilantro::Visualizer viz2(window_name, "disp2");
  viz2.addObject<cilantro::PointCloudRenderable>("cloud_reproj", points_reproj,
                                                 cilantro::RenderingProperties().setPointSize(3.0f))
      ->setPointValues(values);
