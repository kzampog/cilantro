...
int main(int argc, char** argv) {
  ...
  bool randomize = true;
  viz.registerKeyboardCallback(
      'a', std::bind(callback, 'a', std::ref(re_estimate), std::ref(randomize)));
  viz.registerKeyboardCallback(
      'd', std::bind(callback, 'd', std::ref(re_estimate), std::ref(randomize)));
  ...
  while (!viz.wasStopped()) {
    if (randomize) {
      ...
      viz.addObject<cilantro::PointCorrespondencesRenderable>(
          "corr", dst, src, corr, cilantro::RenderingProperties().setOpacity(0.5f));
    ...
    if (re_estimate) {
      ...
      te.setMaxInlierResidual(0.01f)
          .setTargetInlierCount((size_t)(0.50 * corr.size()))
      ...
      std::cout << "RANSAC iterations: " << te.getNumberOfPerformedIterations()
                << ", inlier count: " << te.getNumberOfInliers() << std::endl;
